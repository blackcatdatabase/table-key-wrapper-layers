...
namespace BlackCat\Database\Packages\KeyWrapperLayers\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\KeyWrapperLayers\Definitions;
use BlackCat\Database\Packages\KeyWrapperLayers\Criteria;
use BlackCat\Database\Packages\KeyWrapperLayers\Dto\KeyWrapperLayerDto as Dto;
use BlackCat\Database\Packages\KeyWrapperLayers\Mapper\KeyWrapperLayerDtoMapper as RowMapper;
...
final class KeyWrapperLayerRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\KeyWrapperLayers\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\KeyWrapperLayers\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KeyWrapperLayers\Dto\KeyWrapperLayerDto|null */
    public function getByKeyWrapperIdAndLayerNo(int $keyWrapperId, int $layerNo, bool $asDto = false): array|\BlackCat\Database\Packages\KeyWrapperLayers\Dto\KeyWrapperLayerDto|null {
        $row = $this->getByUnique([ 'key_wrapper_id' => $keyWrapperId, 'layer_no' => $layerNo ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\KeyWrapperLayers\Mapper\KeyWrapperLayerDtoMapper::fromRow($row);
    }
    public function existsByKeyWrapperIdAndLayerNo(int $keyWrapperId, int $layerNo): bool {
        $where = 't.' . Ident::q($this->db, 'key_wrapper_id') . ' = :uniq_key_wrapper_id' . ' AND ' . 't.' . Ident::q($this->db, 'layer_no') . ' = :uniq_layer_no';
        return $this->exists($where, [ 'uniq_key_wrapper_id' => $keyWrapperId, 'uniq_layer_no' => $layerNo ]);
    }
    /** @return int|string|null */
    public function getIdByKeyWrapperIdAndLayerNo(int $keyWrapperId, int $layerNo) {
        $row = $this->getByKeyWrapperIdAndLayerNo($keyWrapperId, $layerNo);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KeyWrapperLayers\Dto\KeyWrapperLayerDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\KeyWrapperLayers\Dto\KeyWrapperLayerDto|null {
