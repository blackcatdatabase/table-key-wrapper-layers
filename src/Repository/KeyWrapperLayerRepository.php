...
use BlackCat\Database\Packages\KeyWrapperLayers\Repository\KeyWrapperLayerRepositoryInterface;
...
class KeyWrapperLayerRepository implements KeyWrapperLayerRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\KeyWrapperLayers\Definitions::class);
    ...
    public function getByKeyWrapperIdAndLayerNo(int $keyWrapperId, int $layerNo, bool $asDto = false): array|\BlackCat\Database\Packages\KeyWrapperLayers\Dto\KeyWrapperLayerDto|null {
        return $this->getByUnique([ 'key_wrapper_id' => $keyWrapperId, 'layer_no' => $layerNo ], $asDto);
    }
    ...
    public function getIdByKeyWrapperIdAndLayerNo(int $keyWrapperId, int $layerNo) {
        $row = $this->getByKeyWrapperIdAndLayerNo($keyWrapperId, $layerNo, false);
