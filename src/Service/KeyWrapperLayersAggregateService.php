...
namespace BlackCat\Database\Packages\KeyWrapperLayers\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\KeyWrapperLayers\Dto\KeyWrapperLayerDto;
use BlackCat\Database\Packages\KeyWrapperLayers\Mapper\KeyWrapperLayerDtoMapper;
 ...
 */
final class KeyWrapperLayersAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private KeyWrapperLayerRepository $keyWrapperLayerRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'key_wrapper_layers' => $this->keyWrapperLayerRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\KeyWrapperLayers\ModuleException("Record {$idStr} not found for locking.");
            }
