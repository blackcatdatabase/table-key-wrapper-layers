...
namespace BlackCat\Database\Packages\KeyWrapperLayers\Joins;
 ...
 */
final class KeyWrapperLayersJoins {
    ...
    /**
     * FK: key_wrapper_layers -> key_wrappers
    ...
    /**
     * FK: key_wrapper_layers -> kms_keys
    ...
    /**
     * FK: key_wrapper_layers -> crypto_algorithms
     * LEFT JOIN vw_crypto_algorithms AS $as ON $as.id = $alias.kem_algo_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinCryptoAlgorithms(string $alias = 't', string $as = 'j2'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_crypto_algorithms AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.kem_algo_id' . ' ', []];
    }
